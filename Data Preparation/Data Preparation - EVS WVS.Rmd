...
```{r}
vdem = read.csv("/Users/sjifradeleeuw/GitHub/Replication-Code-Democratic-Support-And-Left-Right-Dimension/Data/vdem-country_year.csv")
```

Rescale values so that higher values indicate more repression, indoctrination and lack of liberal democracy:
```{r}
rescale = function(x) {
 x = x - min(x, na.rm = TRUE)
...
vdem$indoctrination_r = (rescale(vdem$indoctrination)*-1)+1 
vdem$repression_r = (rescale(vdem$repression)*-1)+1
vdem$electdem_r = (rescale(vdem$electdem)*-1)+1
```
