...
```{r}
library(sp)
germany = readRDS("/Users/sjifradeleeuw/Downloads/gadm36_DEU_1_sp.rds")
```
...
library(ggplot2)
p = ggplot() + 
  geom_polygon(data = subset(coordinates, country != "Germany" & country != "Iceland"), 
    aes(x = long, y = lat, group = country, fill = legacy), colour = "black", size = 0.1) + 
  geom_polygon(data = subset(coordinates, country == "Germany"), 
    aes(x = long, y = lat, group = country), colour = "black", fill = "grey10", size = 0.1, alpha = 1) + 
  geom_polygon(data=
    subset(germany, NAME_1 %in% 
